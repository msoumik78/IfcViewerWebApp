...
(function () {
    ...
    function createChart(container, updateChart) {
        ...
        chart.selectRange('2018-02-14', '2018-12-31');
    ...
    function setDefaultIndicatorSettings() {
       ...
       setInterval(callBackendService, 1000);      
        
        function callBackendService() {
                chart.title('Streaming OHLC Chart for '+$("#chartDataSelect").val());
                $.get('GetStreamingQuote', {
                        'TickerName' : $("#chartDataSelect").val()
                }, function(responseText) {
                        console.log("response from backend :",responseText);
                        ...
                        chartData = [];
                         $.each(info, function(index, element) {
                            chartData.push(info[index]);
                            chartData.splice(1,1,info[index]);
